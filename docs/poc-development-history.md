...
| 항목 | 수치 |
|------|------|
| 전체 기간 | 2026-01-19 ~ 2026-04-15 (약 3개월) |
| 실제 작업일 | **19일** (웹 POC 17일 + domain-analysis 중복 제외 2일) , 실제 투입한 시간 추가 필요 |
| 웹 POC 커밋 | 269건 |
| domain-analysis 커밋 | 28건 |
| 웹 코드량 | 32,429줄 (TSX/TS) |
| 명세서 산출물 | 69,174줄 (67 MD + 142 JSON) |
| 페이지 수 | 36개 |
| 도메인 수 | 26개 |
| 분석된 API | 95개 (93% 분석 완료, 92% 실측 검증) |
| 작업 히스토리 | 214건 (전체 의사결정 추적) |
| WBS 기능 구현율 | 111건 중 80건 완료 (72%) |
...
**3/31 — "스펙 불일치 폭발의 날" (23개 히스토리 엔트리, 하루 최대)**
...
### Phase 5: domain-analysis 3계층 파이프라인 구축 (4/3~4/15)
...
- 완료: 결제 플로우, 취소/환불, 쿠폰 계산, 세션 관리, 친구 추천, 홈 파이프라인 등 **15개 문서 (5,017줄)**
...
### Phase 6: SSoT 기반 웹 POC 품질 개선 (4/3~4/15)
...
| 산출물 | 규모 | 가치 |
|--------|------|------|
| **coolstay-web-nextjs** (웹 POC) | 32,429줄, 36페이지, 26도메인 | 웹 런칭 의사결정 근거 + 본 프로젝트 스펙/프로토타입 |
| **coolstay-domain-analysis** (AI용 명세서) | 69,174줄, 95개 API 분석 | 지금까지 없던 API 명세가 체계적으로 정리됨. 향후 어떤 클라이언트(웹/앱) 개발에서든 재활용 가능 |
...
| 영역 | 구체 사례 |
|------|----------|
| **반복적 UI 구현** | 26개 도메인의 리스트/상세/폼 페이지를 일관된 패턴으로 빠르게 생성 |
...
69,174줄의 명세서를 사람이 직접 작성했다면 수 주가 걸렸을 작업을, AI가 퇴근 후 자율 루프까지 활용하여 약 2주 만에 완료했다.
