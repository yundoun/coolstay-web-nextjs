...
```json
{
  ...
  "books": [
    {
      "book_id": "026033100009",
      "name": "웹부킹",
      "phone_number": "01012341234",
      "safe_number": "050413064024",
      "start_dt": 1774944000,
      "end_dt": 1775016000,
      "reg_dt": 1774940601,
      "vehicle_yn": "N",
      ...
      "origin_price_total": 11000,
      "discount_price_total": 11000,
      "payment": { "method": "SITE", "charge": 11000, "status": "PS003" },
      "motel": {
        "key": "D_KCST_...",
        "partnership_type": "COOLSTAY",
        "business_type": "MOTEL",
        "name": "도운텔2",
        "location": { "address": "부산광역시 중구 광복동1가 61-2" }
      },
      "items": [
        { "key": "D_KCSI_...", "name": "객실명", "category": { "code": "010102", "name": "숙박" } }
      ],
      "refund_policies": [
        { "until": "2026-03-31 17:00:00", "percent": 100, "amount": 11000 }
      ]
    }
  ]
}
...
> **주의**: 모든 타임스탬프(`start_dt`, `end_dt`, `reg_dt`)는 **초 단위**
> `status`는 코드값 (`"BS002"` 등), `payment.status`도 코드값 (`"PS003"`)

#### BookItem 필드

| 필드 | 타입 | 설명 |
|------|------|------|
| `book_id` | string | 예약 ID |
| `name` | string | 예약자 이름 |
| `phone_number` | string | 예약자 전화번호 |
| `safe_number` | string? | 안심번호 |
| `start_dt` | number | 체크인 (초 단위) |
| `end_dt` | number | 체크아웃 (초 단위) |
| `reg_dt` | number | 예약일 (초 단위) |
| `vehicle_yn` | string | 차량 여부 |
| `status` | string | 예약 상태 코드 (`"BS002"` 등) |
| `partial_cancel_yn` | string? | 부분 취소 여부 |
| `partial_refund_yn` | string? | 부분 환불 여부 |
| `refund_yn` | string? | 환불 가능 여부 |
| `total_price` | number | 최종 결제 금액 |
| `discount_price` | number | 할인 적용가 |
| `origin_price_total` | number | 원래 가격 |
| `discount_price_total` | number | 할인 총액 |
| `payment` | object | `{ method, charge, status }` |
| `motel` | object | `{ key, name, partnership_type, business_type, location }` |
| `items` | array | `[{ key, name, category: { code, name } }]` |
| `refund_policies` | array? | `[{ until, percent, amount }]` |
