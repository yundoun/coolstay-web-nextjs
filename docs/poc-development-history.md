...
- **Claude MAX $100/월 요금제**: Opus 모델 5시간 간격으로 사용 가능. Pro($20) 요금제 대비 사용량 5배로, 동일 작업을 Pro로 진행할 경우 **기간도 최소 5배 이상 소요**로 추정.
...
CoolStay 백엔드는 마이크로서비스 구조다. 모바일 앱이 직접 호출하는 API 게이트웨이(`coolstay-service-mobile`)가 있고, 그 뒤에 예약(`coolstay-service-reservation`), 혜택(`coolstay-service-benefit`) 등 내부 서비스가 연결되어 있다.
...
- AOS 앱 코드(Kotlin)는 API를 호출하지만, Retrofit 인터페이스에는 응답 필드의 의미나 비즈니스 규칙이 없다.
...
Phase 3까지 웹 POC에서 겪은 스펙 불일치 (필드명, 타입, Enum 값, 타임스탬프 단위 등)가 모두 이 문서 부재에서 비롯된 것이었다. 하나씩 고치는 것은 끝이 없었고, **"코드를 고치는 것"이 아니라 "정확한 입력(명세)을 만드는 것"이 근본 해결**이라는 판단에서 별도 프로젝트를 신설했다.

`coolstay-domain-analysis`는 웹 POC와 독립된 Git 레포로, AI(Claude Code)가 분석을 수행하고 사람이 결과를 검토하는 구조다. CLAUDE.md에 분석 파이프라인 규칙, 품질 강제 규칙, 슬래시 커맨드(`/analyze`, `/analyze-client`, `/deep-dive`)를 정의하여, 어떤 AI 세션에서 작업하든 동일한 분석 품질을 보장한다.
