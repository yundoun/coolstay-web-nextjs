...
> AI(Claude Code)를 활용한 개발 생산성과 한계를 검증하는 것이 목적이다.
...
CoolStay는 모바일 앱(AOS/iOS)으로 서비스 중인 숙박 예약 플랫폼이다. 웹 서비스 런칭을 검토하는 과정에서, 기존 모바일 API를 재활용하여 AI 도구로 웹 프론트엔드를 구축할 경우 어느 정도의 성과가 나오는지 검증하기 위해 POC를 진행하였다.

### 핵심 전제 조건

- **API 명세서 부재**: 백엔드 API에 대한 공식 문서가 존재하지 않았다. Java/Spring 백엔드 소스코드와 AOS Kotlin 클라이언트 코드, dev 서버만 있는 상태에서 시작하였다.
- **1인 개발**: 프론트엔드 개발자 1명이 Claude Code(AI)와 협업하여 전체 작업을 수행하였다.
- **Claude MAX $100/월 요금제**: Opus 모델 5시간 간격으로 사용 가능. Pro($20) 요금제 대비 사용량 5배로, 동일 작업을 Pro로 진행할 경우 **기간도 최소 5배 이상 소요**될 것으로 추정된다.
...
| 작업 투입 시간 합계 | 약 100시간 (20일 × 하루 평균 5시간), 약 4.2일 |
...
> 1월에 임시 프로젝트로 시작하여 하루 만에 UI 스켈레톤을 구성하고, 이후 약 2개월간 중단하였다.
...
- AI를 활용한 웹 UI 생성 속도에 대한 초기 검증 단계
...
> 2개월 중단 후 본격적으로 POC를 시작하였다.
> AOS 앱과 coolstay-service-mobile 백엔드 코드를 직접 분석하며 전면 재구성하였다.
...
이 시점까지는 코드 기반 추측으로 진행하였으며, 불일치가 발견될 때마다 개별적으로 수정하는 방식이었다.

---

### Phase 3: 인증 연동 + API 불일치 대량 발생 (3/30–3/31, 2일)

> 인증(Auth) 연동에서 심각한 스펙 불일치를 경험하고, 나머지 도메인에서도 연쇄적으로 문제가 발생하였다.

**3/30 — 인증 연동**

로그인 API 호출 시 401 토큰 불일치 에러가 발생하였다. 원인 분석에 상당 시간이 소요되었다.

```
문제: app-secret-code 헤더에 raw secret을 전송하면 401 에러 반환
...
- 이 정보는 어디에도 문서화되어 있지 않았으며, AOS Kotlin 코드를 직접 역추적하여 파악하였다

**3/31 — 스펙 불일치 대량 발생 (30개 히스토리 엔트리, 하루 최대)**

나머지 도메인(마이페이지, 쿠폰, 마일리지, 알림, 기획전 등)을 API 전환하면서 문제가 연쇄적으로 발생하였다:
...
E2E 테스트에서도 추가 이슈가 발견되었다:
- Zustand persist 완료 전 렌더링 → 로그인 상태 false 판정 → 강제 로그아웃
- 예약 목록 데이터 구조 불일치로 렌더링 실패

**이 시점의 핵심 문제 인식**

AOS 코드와 백엔드 Java 코드만으로 추측하는 방식에는 명확한 한계가 있었다. Java 필드명과 JSON 응답 키의 불일치(@JsonNaming 변환), VO에 없는 런타임 필드, V1/V2 Enum 값 불일치 등이 빈번하게 발생하였다. 코드만으로는 70–80%의 정확도가 한계이며, 나머지 20–30%의 불일치가 전체 구현의 안정성을 저해하는 핵심 요인이었다.

---

### Phase 4: API 응답 기반 전면 재설계 + 전략 전환 (4/1–4/3, 3일)

> dev 서버의 실제 응답을 기준으로 타입과 UI를 전면 재설계하면서,
> **입력 데이터(API 명세)의 품질을 높이는 것이 핵심**이라는 판단에 도달하였다.

**4/1 — API 응답 기반 UI 재설계**

dev 서버 실제 응답을 기준으로 타입과 UI를 전면 재설계하였다:
...
수동으로 API 응답을 확인하고 타입을 수정하는 작업이 반복되면서, 근본적인 접근 방식의 전환이 필요하다는 결론에 이르렀다.

- AOS 코드 + 백엔드 코드 기반 추측 방식의 한계가 명확해짐
- 코드만으로는 70–80%의 정확도가 한계이며, 나머지 20–30%의 불일치가 전체 구현의 재작업을 유발
- **코드를 수정하는 것이 아니라, 정확한 명세를 구축하는 것이 근본 해결책**이라는 결론

→ 4/3, **coolstay-domain-analysis** 프로젝트를 신규 생성하고 체계적인 역분석 파이프라인 구축에 착수하였다.
...
> 단일 참조점(SSoT)을 구축하는 파이프라인이다.

#### 배경: 별도 프로젝트가 필요한 이유

CoolStay 백엔드는 마이크로서비스 구조이다. 모바일 앱이 직접 호출하는 API 게이트웨이(`coolstay-service-mobile`)가 있고, 그 뒤에 예약(`coolstay-service-reservation`), 혜택(`coolstay-service-benefit`) 등 내부 서비스가 연결되어 있다.

문제는 이 구조의 **어디에도 API 명세서가 없다**는 점이다.

- 백엔드 서버 코드(Java/Spring)는 있지만, Java 필드명이 JSON 응답 키와 다르다 (`@JsonNaming(SnakeCaseStrategy)` 적용, `@JsonProperty` 오버라이드).
- AOS 앱 코드(Kotlin)는 API를 호출하지만, Retrofit 인터페이스에는 응답 필드의 의미나 비즈니스 규칙이 포함되어 있지 않다.
- V1/V2 버전이 혼재하고, 같은 엔드포인트도 파라미터 조합에 따라 응답 구조가 달라진다.
- 인증 헤더(`app-secret-code`)는 AES-128-CBC 암호화가 필요한데, 이 사실은 AOS의 `EncryptUtil.kt`를 역분석해야만 알 수 있었다.

Phase 3까지 웹 POC에서 겪은 스펙 불일치(필드명, 타입, Enum 값, 타임스탬프 단위 등)가 모두 이 문서 부재에서 비롯된 것이었다. 개별 수정으로는 해결이 불가능하였으며, **정확한 입력(명세)을 구축하는 것이 근본 해결책**이라는 판단에서 별도 프로젝트를 신설하였다.

`coolstay-domain-analysis`는 웹 POC와 독립된 Git 레포로, AI(Claude Code)가 분석을 수행하고 사람이 결과를 검토하는 구조이다. CLAUDE.md에 분석 파이프라인 규칙, 품질 강제 규칙, 슬래시 커맨드(`/analyze`, `/analyze-client`, `/deep-dive`)를 정의하여, 어떤 AI 세션에서 작업하든 동일한 분석 품질을 보장한다.
...
| **Layer 2: Verifier** | dev 서버 실측 검증 | 정적 분석만으로는 70–80%의 정확도. **Java 필드명 ≠ JSON 키**, VO에 없는 런타임 필드 등 나머지 20–30%를 실측으로 보완 |
| **Layer 3: Client Analyzer** | AOS 앱 교차검증 | API의 실제 사용 방식을 Kotlin 코드에서 추출. 호출 시퀀스, 파라미터 조합, 필드 사용률, 에러 핸들링 |
...
서버 소스코드를 읽기 전용으로 분석하여 API 명세 초안을 생성한다.

- **Controller 계층**: 엔드포인트 경로, HTTP 메서드, 파라미터, @ApiResponse 어노테이션
- **Service 계층**: if/else 분기 전수 조사, throw문 → 에러코드 매핑, 한국어 주석 확인
...
dev 서버에 실제 API 호출을 수행하고, Layer 1 초안과의 diff를 자동 검증한다.

- `node scripts/verify-diff.mjs {domain}`으로 자동 diff 검증 수행
- 파라미터 조합별 전수 호출 (같은 엔드포인트라도 type/category에 따라 응답 구조가 상이)
- diff 리포트의 **[UNDOC]** (문서에 없는 필드), **[PHANTOM]** (문서에만 있는 필드), **[MISMATCH]** (타입/값 불일치) 전부 해소 후 분석 완료
...
AOS 앱의 Kotlin 코드를 추적하여 API의 실제 사용 방식을 검증한다.
...
복잡한 비즈니스 로직이 있는 도메인에 대해 선택적으로 실행한다.

- 3 Phase 루프 (Reader → Tracer → Documenter)
- 대상 기준: 100줄 이상 메서드, 3단계 이상 분기 중첩, 3개 이상 외부 서비스 호출
- 완료: 결제 플로우, 취소/환불, 쿠폰 계산, 세션 관리, 친구 추천, 홈 파이프라인 등 **16개 문서 (5,261줄)**
...
이 작업을 업무 시간에만 진행할 경우 토큰 한도와 시간 모두 부족하다.
Claude Code의 **자율 루프(autonomous loop)** 기능을 활용하여 퇴근 후에도 분석 파이프라인을 지속 실행하였다.

```
운영 패턴:
1. 업무 시간: 분석 방향 설정, 결과 검토, SSoT 수정 판단
2. 퇴근 후: /analyze <domain>, /analyze-client <domain> 자율 루프 실행
3. 다음 출근: 야간 생성 산출물 검토 → 수정 → 다음 도메인 진행
...
domain-analysis 프로젝트의 CLAUDE.md에 아래 규칙을 정의하여, AI가 어떤 세션에서 작업하든 동일한 품질 기준을 유지하도록 강제하였다:
...
2. 실측 검증 규칙 (최우선)
   - verify-diff.mjs 자동 검증 필수 (수동 대조 금지)
   - diff 리포트의 [UNDOC], [PHANTOM], [MISMATCH] 전부 해소 후 완료
   - Java 필드명 ≠ JSON 키 (@JsonNaming, @JsonProperty 주의)
   - 파라미터 조합별 전수 호출 (같은 엔드포인트라도 응답 구조가 상이할 수 있음)

3. 분석 품질 규칙
   - if/else 분기 전수 조사 (결제수단별, 금액별, 사용자타입별)
   - 한국어 주석 확인 필수 (필드의 실제 의미가 주석에 있는 경우 다수)
...
> domain-analysis가 확립된 후, 웹 POC의 구현 속도와 정확도가 동시에 향상되었다.
...
AI가 코드를 생성할 때 일관된 품질을 유지하도록 프로젝트에 설정한 규칙과 도구 체계이다.
...
API 명세를 역분석하는 AI 전용 프로젝트이다. CLAUDE.md에 분석 파이프라인 규칙과 스킬(슬래시 커맨드)을 정의하였다.
...
│   ├── verify-diff.mjs 실행 필수 (수동 대조 금지)
...
2월부터 운영 중인 Vite 기반 문서 사이트이다. 4/6에 domain-analysis 결과를 연동하여 사람이 읽을 수 있는 형태로 변환하였다.

- 154개 콘텐츠 파일 (CMS/PMS 권한, 정책, 기획서 링크, API 명세)
- domain-analysis가 AI용 SSoT라면, coolstay-docs는 **사내 공유용 SSoT**에 해당한다

---

## 4. 산출물 정리

이 POC의 산출물은 웹 프로토타입 하나가 아니라 **3개**이다.

| 산출물 | 규모 | 가치 |
|--------|------|------|
| **coolstay-web-nextjs** (웹 POC) | 32,669줄, 36페이지, 25도메인 | 웹 런칭 의사결정 근거 + 본 프로젝트 스펙/프로토타입 |
| **coolstay-domain-analysis** (AI용 명세서) | 69,562줄, 95개 API 역분석, 117개 실측 JSON, 16개 심층 문서 | 지금까지 없던 API 명세가 체계적으로 정리됨. 백엔드 코드 정적분석 + dev 서버 실측 + AOS 교차검증의 3계층 파이프라인으로 구축. 향후 어떤 클라이언트(웹/앱) 개발에서든 재활용 가능 |
| **coolstay-docs** (사내 공유 문서) | 154개 콘텐츠 파일 | 팀 내 도메인 지식 공유 및 신규 인력 온보딩 자료 |
...
| **역분석 파이프라인** | Java/Kotlin 소스를 읽고 구조화된 명세서로 변환 (수동 작업 시 수 주 소요 예상) |
...
| **비즈니스 분기 대응** | 현장결제/카드결제 분기, 취소 정책 적용 등 도메인 지식이 필요한 판단 |
| **API 스펙 불일치 대응** | 실측 없이 코드만 분석할 경우 70–80% 정확도 한계. 반드시 실측 검증이 필요 |
| **SDK 연동** | Kakao/Naver OAuth, Naver Map 임베디드, NICE 본인인증 등 외부 SDK는 AI만으로 완성 불가 |

### 핵심 교훈

> **AI에게 "코드를 수정하라"보다 "명세를 만들어라"가 훨씬 효과적이다.**

Phase 3까지는 API 불일치가 발생할 때마다 코드를 수정하는 방식이었으나, 근본적인 해결이 되지 않았다.
domain-analysis 파이프라인을 구축하여 **입력의 품질을 높이자**, 출력(웹 POC)의 품질이 동반 상승하였다.

- 명세 없이 코드 추측 → 구현 → 불일치 발견 → 수정: **재작업의 악순환**
- 3계층 역분석 → 정확한 명세 → 구현: **한 번에 정확한 구현**

AI의 가장 큰 가치는 코드를 대신 작성하는 것이 아니라,
**수동으로 수행 시 수 주가 소요되는 역분석/문서화를 체계적으로 수행하는 것**이었다.
69,562줄의 명세서를 사람이 직접 작성하였다면 수 주가 소요되었을 작업을, AI가 퇴근 후 자율 루프까지 활용하여 약 2주 만에 완료하였다.
...
Claude 공식 기준 MAX는 Pro 대비 사용량 5배이다. 동일 작업을 Pro로 진행할 경우:
...
### 추가 개발 필요 기능 및 미구현 항목

| 항목 | 설명 |
|------|------|
| **카카오/네이버 SNS 연동** | OAuth SDK 연동 필요. 백엔드 API(`/auth/sessions/users/sns`)는 분석 완료되었으나, 클라이언트 측 SDK 초기화 및 토큰 교환 플로우 미구현 |
| **친구추천** | 추천 코드 생성/등록 API(`/auth/users/friend/register`, `/auth/users/friend/list`)는 분석 완료. UI 및 공유 기능 미구현 |
| **마일리지 적립/사용** | 마일리지 조회는 구현 완료. 예약 시 마일리지 적용 로직 및 적립 내역 연동 미구현 |
| **매거진** | API 분석 완료(8개 엔드포인트). 매거진 홈/보드/패키지 페이지 라우트는 존재하나 실제 API 연동 미완료 |
| **네이버 지도** | 숙소 상세 페이지 내 위치 표시용. Naver Maps JavaScript SDK 임베디드 연동 필요 |
| **기획전 2차 고도화** | 기획전 목록/상세 기본 구현 완료. 패키지형 기획전 하위 숙소 목록, board_type별 분기 렌더링, 쿠폰 다운로드 연동 등 고도화 필요 |
